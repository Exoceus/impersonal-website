...
export default function Lever({setLeverPulled}) {
    ...
    const bind = useDrag(
        ({movement: [x, y], timeStamp, event, active}) => {
            ...
            let new_y = 5 + (-y / aspect) * 2; // times 4 since we scale by 0.25
            if (new_y <= -5) {
                setLeverPulled(true);
                new_y = -5;
            } else {
    ...
    useEffect(() => {
        if (!isDragging || position.slice().sort().join(",") !== [0, 5, 0].slice().sort().join(",")) {
            setPosition([0, 5, 0]);
        }
        console.log("CHANGE", isDragging);
    }, [isDragging]);

    const {nodes, materials} = useGLTF("models/Lever_Full_Model-transformed.glb");
    return (
        <group
            rotation={[(0 * Math.PI) / 180, (290 * Math.PI) / 180, 0]}
            scale={0.25}
            dispose={null}
            position={[1, 0, 2]}
        >
            <group ref={ref} position={position} {...bind()}>
                <mesh
                    ...
                    material={materials["Lever Shaft"]}
